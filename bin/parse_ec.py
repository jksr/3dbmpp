...
def get_strand_ranges(strandfn):
	ranges = []
	f = open(strandfn)
	lines = f.readlines()
	f.close()
...
# this function is similar with  get_strand_ranges(strandfn)
def get_circular_barrel(strandfn):
	strand_ranges = get_strand_ranges(strandfn)
	barrel = []
...
#}
def get_neighbor_ec_dict(strandfn, ecfn):
	strand_ranges = get_strand_ranges(strandfn)
